...
/* Stack bounds checking for debugging */
#ifdef CONFIG_STACK_SENTINEL

void z_arch_stack_bounds_check(struct k_thread *thread)
{
	uint32_t *sentinel = (uint32_t *)thread->stack_info.start;

	if (*sentinel != STACK_SENTINEL_VALUE) {
		printk("Stack sentinel corrupted in thread %p!\n", thread);
		printk("Expected: 0x%08x, Found: 0x%08x\n", STACK_SENTINEL_VALUE, *sentinel);
		k_panic();
	}
}
