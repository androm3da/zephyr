...
/* Callee-saved register buffer shared with assembly (event_handlers.S) */
uint32_t _gdb_callee_regs[GDB_CALLEE_BUF_SIZE];
...
/**
 * Core entry point from assembly debug handler or exception handler.
 *
 * Copies register state from the event_context (volatile regs saved by
 * assembly) and the static _gdb_callee_regs buffer (callee-saved regs)
 * into the GDB context, calls the GDB main loop, then copies any
 * modifications back so the assembly restore path picks them up.
 */
void z_hexagon_gdb_entry(struct event_context *ctx)
{
	...
	/* --- Copy callee-saved registers from static buffer --- */
	regs[GDB_HEXAGON_R16] = _gdb_callee_regs[0];
	regs[GDB_HEXAGON_R17] = _gdb_callee_regs[1];
	regs[GDB_HEXAGON_R18] = _gdb_callee_regs[2];
	regs[GDB_HEXAGON_R19] = _gdb_callee_regs[3];
	regs[GDB_HEXAGON_R20] = _gdb_callee_regs[4];
	regs[GDB_HEXAGON_R21] = _gdb_callee_regs[5];
	regs[GDB_HEXAGON_R22] = _gdb_callee_regs[6];
	regs[GDB_HEXAGON_R23] = _gdb_callee_regs[7];
	regs[GDB_HEXAGON_R24] = _gdb_callee_regs[8];
	regs[GDB_HEXAGON_R25] = _gdb_callee_regs[9];
	regs[GDB_HEXAGON_R26] = _gdb_callee_regs[10];
	regs[GDB_HEXAGON_R27] = _gdb_callee_regs[11];
	regs[GDB_HEXAGON_R28] = _gdb_callee_regs[12];
	...
	/* Special registers from callee-save buffer */
	regs[GDB_HEXAGON_USR] = _gdb_callee_regs[13];
	regs[GDB_HEXAGON_GP]  = _gdb_callee_regs[14];
	regs[GDB_HEXAGON_UGP] = _gdb_callee_regs[15];
	regs[GDB_HEXAGON_LC0] = _gdb_callee_regs[16];
	regs[GDB_HEXAGON_LC1] = _gdb_callee_regs[17];
	regs[GDB_HEXAGON_SA0] = _gdb_callee_regs[18];
	regs[GDB_HEXAGON_SA1] = _gdb_callee_regs[19];
	...
	/* Callee-saved GPRs back to static buffer (assembly will restore) */
	_gdb_callee_regs[0]  = regs[GDB_HEXAGON_R16];
	_gdb_callee_regs[1]  = regs[GDB_HEXAGON_R17];
	_gdb_callee_regs[2]  = regs[GDB_HEXAGON_R18];
	_gdb_callee_regs[3]  = regs[GDB_HEXAGON_R19];
	_gdb_callee_regs[4]  = regs[GDB_HEXAGON_R20];
	_gdb_callee_regs[5]  = regs[GDB_HEXAGON_R21];
	_gdb_callee_regs[6]  = regs[GDB_HEXAGON_R22];
	_gdb_callee_regs[7]  = regs[GDB_HEXAGON_R23];
	_gdb_callee_regs[8]  = regs[GDB_HEXAGON_R24];
	_gdb_callee_regs[9]  = regs[GDB_HEXAGON_R25];
	_gdb_callee_regs[10] = regs[GDB_HEXAGON_R26];
	_gdb_callee_regs[11] = regs[GDB_HEXAGON_R27];
	_gdb_callee_regs[12] = regs[GDB_HEXAGON_R28];

	/* Special registers back to static buffer */
	_gdb_callee_regs[13] = regs[GDB_HEXAGON_USR];
	_gdb_callee_regs[14] = regs[GDB_HEXAGON_GP];
	_gdb_callee_regs[15] = regs[GDB_HEXAGON_UGP];
	_gdb_callee_regs[16] = regs[GDB_HEXAGON_LC0];
	_gdb_callee_regs[17] = regs[GDB_HEXAGON_LC1];
	_gdb_callee_regs[18] = regs[GDB_HEXAGON_SA0];
	_gdb_callee_regs[19] = regs[GDB_HEXAGON_SA1];
}
