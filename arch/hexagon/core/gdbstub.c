...
size_t arch_gdb_reg_readall(struct gdb_ctx *ctx, uint8_t *buf, size_t buf_size)
{
	...
	/* Pack registers into GDB packet format */
	for (int i = 0; i < GDB_HEXAGON_NUM_REGS; i++) {
		if (pos + 8 > buf_size) {
			break;
		}
		...
		for (int j = 0; j < 8; j++) {
			uint8_t nibble = (ctx->regs[i] >> ((7 - j) * 4)) & 0xF;
...
size_t arch_gdb_reg_writeall(struct gdb_ctx *ctx, uint8_t *buf, size_t buf_size)
{
	...
	/* Unpack registers from GDB packet */
	for (int i = 0; i < GDB_HEXAGON_NUM_REGS && pos + 8 <= buf_size; i++) {
		uint32_t value = 0;
