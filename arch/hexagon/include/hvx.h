...
/* Byte offset of vecpredsave within struct hvx_vectors (= 32 * HVX_VECTOR_SIZE) */
#define HVX_VECPRED_OFFSET (32 * HVX_VECTOR_SIZE)
...
/**
 * @brief Context-switch hook: save old thread's HVX state and restore new thread's.
 *
 * Must be called on every context switch.  old_thread is the thread being
 * switched away from; new_thread is the thread being switched to (_current
 * after the stack swap).  Safe to call when neither thread has an HVX
 * context allocated.
 *
 * @param old_thread Thread being switched away from
...
struct k_thread;
static inline void hvx_arch_thread_switch(struct k_thread *old_thread,
