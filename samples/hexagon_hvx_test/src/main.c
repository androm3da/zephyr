...
static void test_hvx_availability(void)
{
	...
	if (hvx_is_available()) {
		printf("[PASS] HVX is available on this hardware\n");
	} else {
		printf("[FAIL] HVX is not available on this hardware\n");
		return;
	}
...
static void test_hvx_context_management(void)
{
	...
	if (ret == 0) {
		printf("[PASS] HVX context allocated successfully\n");
	} else {
		printf("[FAIL] HVX context allocation failed: %d\n", ret);
		return;
	}
	...
	if (ctx != NULL) {
		printf("[PASS] HVX context retrieved: context_num=%d\n", ctx->context_num);
	} else {
		printf("[FAIL] Failed to retrieve HVX context\n");
	}

	/* Test context save/restore */
	ret = hvx_context_save();
	if (ret == 0) {
		printf("[PASS] HVX context saved successfully\n");
	} else {
		printf("[FAIL] HVX context save failed: %d\n", ret);
	}

	ret = hvx_context_restore();
	if (ret == 0) {
		printf("[PASS] HVX context restored successfully\n");
	} else {
		printf("[FAIL] HVX context restore failed: %d\n", ret);
	}

	/* Free context */
	hvx_context_free();
	printf("[PASS] HVX context freed\n");
...
static void test_hvx_enable_disable(void)
{
	...
	ret = hvx_enable();
	if (ret == 0) {
		printf("[PASS] HVX enabled successfully\n");
	} else {
		printf("[FAIL] HVX enable failed: %d\n", ret);
		return;
	}
	...
	if (ctx != NULL) {
		printf("[PASS] HVX context automatically allocated: context_num=%d\n",
		       ctx->context_num);
	} else {
		printf("[FAIL] HVX context not allocated after enable\n");
	}

	/* Disable HVX */
	hvx_disable();
	printf("[PASS] HVX disabled successfully\n");
...
static void test_hvx_multiple_contexts(void)
{
	...
	/* Try to allocate contexts until we run out */
	for (int i = 0; i < 10; i++) {
		ret = hvx_context_alloc();
		if (ret == 0) {
			allocated_contexts++;
			printf("[PASS] Context %d allocated\n", i);
		} else {
			printf("[INFO] Context %d allocation failed: %d\n", i, ret);
			break;
		}
	}

	printf("Total contexts allocated: %d\n", allocated_contexts);
...
static void test_hvx_vector_operations(void)
{
	...
	ret = hvx_enable();
	if (ret != 0) {
		printf("[FAIL] Failed to enable HVX for vector operations: %d\n", ret);
		return;
	}

	printf("[PASS] HVX enabled for vector operations\n");
	...
	ret = hvx_context_save();
	if (ret == 0) {
		printf("[PASS] HVX context saved after operations\n");
	} else {
		printf("[FAIL] HVX context save failed: %d\n", ret);
	}

	hvx_disable();
	printf("[PASS] HVX disabled after operations\n");
